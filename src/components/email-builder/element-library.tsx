...
export function ElementLibrary({ onAdd, compact = false }: { onAdd: (type: EmailBlockType) => void; compact?: boolean }) {
  const { t } = useI18n();
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-card">
      ...
      <div className={compact ? "flex min-h-0 flex-1 flex-col" : "grid min-h-0 flex-1 grid-cols-[5.5rem_1fr]"}>
        <div className={compact ? "flex gap-1 overflow-x-auto border-b p-2" : "flex flex-col gap-0.5 border-e p-2"} role="list" aria-label="Element categories">
          {categories.map((item) => (
            <button
              key={item}
              type="button"
        ...
        <div className={`grid content-start gap-2 overflow-y-auto p-3 ${compact ? "grid-cols-3 sm:grid-cols-4" : "grid-cols-2"}`}>
          {filtered.map((item) => {
            ...
            return (
              <button
                ...
                className="group flex min-h-20 flex-col items-center justify-center gap-2 rounded-md border bg-background px-2 py-3 text-center outline-none transition-colors hover:border-primary/50 hover:bg-primary/5 focus-visible:ring-2 focus-visible:ring-ring"
