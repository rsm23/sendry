...
function MessageBubble({ message }: { message: Message }) {
  ...
  return <div className={cn('flex gap-2', outbound && 'justify-end')}>
    ...
    <div className={cn('max-w-[78%]', outbound && 'text-end')}>
      ...
      <div className={cn('rounded-xl border px-3 py-2 text-start text-[0.8rem] leading-snug shadow-[0_1px_0_rgba(15,23,42,.03)] md:px-3.5 md:py-2.5 md:text-sm md:leading-relaxed', outbound ? 'border-blue-200 bg-blue-50' : message.channel === 'whatsapp' ? 'border-emerald-200 bg-emerald-50/70' : 'bg-card')}>{message.body}{outbound && <CheckCheck className="ms-auto mt-1 size-3.5 text-primary"/>}</div>
    </div>
  </div>
}
