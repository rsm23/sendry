...
export default function FilesPage() {
  ...
  function itemAction(item: FileItem, action: string) {
    ...
    if (action === 'download') {
      const path = item.kind === 'folder' ? `/api/brands/${brand?.id}/files/bulk/download?ids=${encodeURIComponent(item.id)}` : `/api/brands/${brand?.id}/files/${item.id}/content?download=1`
  ...
  return <div ref={workspace} tabIndex={-1} onKeyDown={onKeyDown} className="flex h-[calc(100svh-6.5rem)] min-h-[34rem] flex-col overflow-hidden rounded-xl border bg-card shadow-sm outline-none">
    ...
    <div className="hidden min-h-0 flex-1 lg:block"><ResizablePanelGroup orientation="horizontal" className="min-h-0"><ResizablePanel id="files-main" minSize="55%"><WorkspaceBody view={view} params={params} crumbs={crumbs} currentFolder={parent.data} files={files.data ?? []} loading={files.isLoading} error={files.error} viewMode={viewMode} selection={selection} onSelect={select} onOpen={openItem} onAction={itemAction} onClearSelection={() => setSelection(new Set())} draggedId={draggedId} setDraggedId={setDraggedId} onMove={async (id, target) => { await bulk('move', [id], target) }} dropActive={dropActive} setDropActive={setDropActive} onUpload={uploadSelected} onUploadFiles={askFilesUpload} onUploadFolder={askFolderUpload} onCreateFolder={() => setFolderDialog(true)} /></ResizablePanel>{selected && detailsOpen ? <><ResizableHandle withHandle /><ResizablePanel id="files-details" defaultSize={`${preferences.data?.details_width ?? 360}px`} minSize="300px" maxSize="520px"><FileDetailsPanel key={`${selected.id}:${detailsTab}`} brandId={brand?.id ?? ''} item={selected} onClose={() => setDetailsOpen(false)} initialTab={detailsTab} onChanged={() => void refresh()} /></ResizablePanel></> : null}</ResizablePanelGroup></div>
    <div className="min-h-0 flex-1 lg:hidden"><WorkspaceBody view={view} params={params} crumbs={crumbs} currentFolder={parent.data} files={files.data ?? []} loading={files.isLoading} error={files.error} viewMode={viewMode} selection={selection} onSelect={select} onOpen={openItem} onAction={itemAction} onClearSelection={() => setSelection(new Set())} draggedId={draggedId} setDraggedId={setDraggedId} onMove={async (id, target) => { await bulk('move', [id], target) }} dropActive={dropActive} setDropActive={setDropActive} onUpload={uploadSelected} onUploadFiles={askFilesUpload} onUploadFolder={askFolderUpload} onCreateFolder={() => setFolderDialog(true)} /></div>
...
function WorkspaceBody({ view, params, crumbs, currentFolder, files, loading, error, viewMode, selection, onSelect, onOpen, onAction, onClearSelection, draggedId, setDraggedId, onMove, dropActive, setDropActive, onUpload, onUploadFiles, onUploadFolder, onCreateFolder }: { view: string; params: URLSearchParams; crumbs: Array<{ id: string; name: string }>; currentFolder?: FileItem; files: FileItem[]; loading: boolean; error: Error | null; viewMode: ViewMode; selection: Set<string>; onSelect: (item: FileItem, index: number, event?: MouseEvent) => void; onOpen: (item: FileItem) => void; onAction: (item: FileItem, action: string) => void; onClearSelection: () => void; draggedId: string | null; setDraggedId: (id: string | null) => void; onMove: (id: string, target: string) => Promise<void>; dropActive: boolean; setDropActive: (value: boolean) => void; onUpload: (files: FileList | File[]) => Promise<void>; onUploadFiles: () => void; onUploadFolder: () => void; onCreateFolder: () => void }) {
  const { t } = useI18n()
  const navigate = useNavigate()
  ...
  const canvas = <div className="min-h-full p-3" data-testid="folder-canvas">{loading ? <FilesSkeleton mode={viewMode} /> : error ? <Alert variant="destructive"><ArchiveRestore /><AlertTitle>{t('Files could not be loaded')}</AlertTitle><AlertDescription>{error.message}</AlertDescription></Alert> : !files.length ? <EmptyFiles view={view} onUpload={onUploadFiles} onCreateFolder={onCreateFolder} /> : viewMode === 'grid' ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{files.map((item, index) => <GridItem key={item.id} item={item} index={index} selected={selection.has(item.id)} onSelect={onSelect} onOpen={onOpen} onAction={onAction} setDraggedId={setDraggedId} draggedId={draggedId} onMove={onMove} />)}</div> : <ListItems compact={viewMode === 'compact'} files={files} selection={selection} onSelect={onSelect} onOpen={onOpen} onAction={onAction} setDraggedId={setDraggedId} draggedId={draggedId} onMove={onMove} />}</div>
  return <div className="flex h-full min-h-0"><aside className="hidden w-48 shrink-0 border-e bg-muted/20 p-2 lg:block"><nav aria-label={t('File views')} className="space-y-1">{views.map((item) => <Button key={item.id} variant={view === item.id ? 'secondary' : 'ghost'} className="w-full justify-start" onClick={() => { const next = new URLSearchParams(params); if (item.id === 'all') next.delete('view'); else next.set('view', item.id); next.delete('parentId'); navigate(`/files?${next}`) }}><item.icon />{t(item.label)}</Button>)}</nav><div className="mt-6 rounded-lg border bg-card p-3 text-xs"><div className="mb-2 flex items-center gap-2"><HardDriveUpload className="size-4 text-primary" /><strong>{t('Secure library')}</strong></div><p className="text-muted-foreground">{t('100 MB per file · quarantine and malware scan')}</p></div></aside><main className="relative flex min-w-0 flex-1 flex-col" onDragEnter={(event) => { if (event.dataTransfer.types.includes('Files')) setDropActive(true) }} onDragOver={(event) => { if (event.dataTransfer.types.includes('Files')) event.preventDefault() }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDropActive(false) }} onDrop={drop}><div className="flex h-11 items-center gap-2 border-b px-3"><Breadcrumb className="min-w-0 flex-1"><BreadcrumbList className="flex-nowrap overflow-hidden"><BreadcrumbItem><BreadcrumbLink render={<Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => { const next = new URLSearchParams(params); next.delete('parentId'); next.delete('view'); navigate(`/files?${next}`) }} />}>{t('All files')}</BreadcrumbLink></BreadcrumbItem>{crumbs.map((crumb, index) => <span key={crumb.id} className="contents"><BreadcrumbSeparator /><BreadcrumbItem className="min-w-0">{index === crumbs.length - 1 ? <BreadcrumbPage className="truncate" translate="no">{crumb.name}</BreadcrumbPage> : <BreadcrumbLink render={<Button size="sm" variant="ghost" className="h-7 max-w-40 truncate px-2" onClick={() => { const next = new URLSearchParams(params); next.set('parentId', crumb.id); navigate(`/files?${next}`) }} />}>{crumb.name}</BreadcrumbLink>}</BreadcrumbItem></span>)}</BreadcrumbList></Breadcrumb><span className="text-xs text-muted-foreground">{t('{count} items', { count: files.length })}</span></div><div className="min-h-0 flex-1 overflow-auto">{view === 'all' ? <FolderContextMenu folder={currentFolder} onClearSelection={onClearSelection} onCreateFolder={onCreateFolder} onUploadFiles={onUploadFiles} onUploadFolder={onUploadFolder} onAction={onAction}>{canvas}</FolderContextMenu> : canvas}</div>{dropActive ? <div className="pointer-events-none absolute inset-3 z-30 grid place-items-center rounded-xl border-2 border-dashed border-primary bg-primary/10 backdrop-blur-sm"><div className="text-center"><Upload className="mx-auto size-10 text-primary" /><strong className="mt-2 block">{t('Drop files to upload')}</strong><span className="text-sm text-muted-foreground">{t('They will be added to the current folder.')}</span></div></div> : null}</main></div>
}

function FolderContextMenu({ folder, onClearSelection, onCreateFolder, onUploadFiles, onUploadFolder, onAction, children }: { folder?: FileItem; onClearSelection: () => void; onCreateFolder: () => void; onUploadFiles: () => void; onUploadFolder: () => void; onAction: (item: FileItem, action: string) => void; children: React.ReactElement }) {
  const { t } = useI18n()
  const canEdit = !folder || ['editor', 'manager'].includes(folder.effective_role)
  return <ContextMenu onOpenChange={(open) => { if (open) onClearSelection() }}><ContextMenuTrigger render={children} /><ContextMenuContent><ContextMenuGroup><ContextMenuLabel translate={folder ? 'no' : undefined}>{folder?.name ?? t('All files')}</ContextMenuLabel>{canEdit ? <><ContextMenuItem onClick={onCreateFolder}><FolderPlus />{t('New folder')}</ContextMenuItem><ContextMenuItem onClick={onUploadFiles}><Upload />{t('Upload files')}</ContextMenuItem><ContextMenuItem onClick={onUploadFolder}><FolderInput />{t('Upload folder')}</ContextMenuItem></> : null}</ContextMenuGroup>{folder ? <><ContextMenuSeparator /><ContextMenuGroup><ContextMenuItem onClick={() => onAction(folder, 'download')}><Download />{t('Download folder')}</ContextMenuItem><ContextMenuItem onClick={() => onAction(folder, 'details')}><Info />{t('Folder information')}</ContextMenuItem>{folder.effective_role === 'manager' ? <ContextMenuItem onClick={() => onAction(folder, 'share')}><Share2 />{t('Share folder')}</ContextMenuItem> : null}</ContextMenuGroup></> : null}</ContextMenuContent></ContextMenu>
}
...
function EmptyFiles({ view, onUpload, onCreateFolder }: { view: string; onUpload: () => void; onCreateFolder: () => void }) {
  const { t } = useI18n()
  const content = ({ trash: [Trash2, t('Trash is empty'), t('Items moved to Trash appear here until they are restored or deleted forever.')], starred: [Star, t('No starred files'), t('Add a star to keep important files close.')], shared: [Share2, t('Nothing shared yet'), t('Restricted items and active external links appear here.')], recent: [Clock3, t('No recent files'), t('Files you upload or update will appear here.')] } as Record<string, [typeof Trash2, string, string]>)[view] ?? [HardDriveUpload, t('Build your file library'), t('Upload documents, images, spreadsheets, presentations, code, or create a folder.')]
  const Icon = content[0]
  return <div className="grid min-h-80 place-items-center rounded-xl border border-dashed bg-muted/10 p-8 text-center"><div><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-muted"><Icon className="size-7 text-muted-foreground" /></span><h2 className="mt-4 font-medium">{content[1]}</h2><p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">{content[2]}</p>{view === 'all' ? <div className="mt-4 flex justify-center gap-2"><Button variant="outline" onClick={onCreateFolder}><FolderPlus />{t('New folder')}</Button><Button onClick={onUpload}><Upload />{t('Upload files')}</Button></div> : null}</div></div>
}
