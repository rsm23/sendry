...
- Drive-style brand File workspace with All, Recent, Starred, Shared, and Trash views; recursive search; filters; sorting; grid/list/compact modes; breadcrumbs; drag/drop; keyboard navigation; multi-selection; empty-space folder context actions; recursive streamed ZIP, move, copy, star, trash, and restore
...
- Viewer/commenter/editor/manager ACLs, default brand visibility, restricted inherited subtrees, external file/folder links with optional expiry/password/recursive ZIP download, access logs, rate limits, persistent notifications, and ACL-checked live file rooms
